...
[存储引擎](https://blog.csdn.net/qq_35642036/article/details/89554721)
...
在MySQL中，如果不显式设置隔离级别，则默认采用的隔离级别是可重复读（REPEATABLE READ）。在<font color=#F09B59 style=" font-weight:bold;">可重复读隔离级别下，读锁和写锁的行为</font>如下：

1. **读锁**：
   - 在可重复读隔离级别下，读锁会自动加上，允许多个事务同时读取同一份数据。
   - 如果一个事务正在读取某一数据时，其他事务也可以读取相同的数据，而不会被阻塞。
   - 读锁不会阻止其他事务获取数据的读锁，但会阻止其他事务获取数据的写锁。

2. **写锁**：
   - 在可重复读隔离级别下，写锁会在执行写操作时自动加上，确保只有一个事务能够对数据进行修改。
   - 如果一个事务持有了数据的写锁，其他事务无法获取该数据的读锁或写锁，直到写锁被释放。
   - 事务在持有写锁期间，其他事务的读操作都会被阻塞，以保证数据的一致性和完整性。

总的来说，在MySQL默认使用可重复读隔离级别时，读锁和写锁的行为是允许多个事务同时读取数据，但只允许一个事务对数据进行修改，并且在该事务对数据进行修改期间，其他事务的读操作会被阻塞。这样可以确保数据的并发访问时不会出现问题，保证数据的一致性。
